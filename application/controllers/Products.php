...
class Products extends CI_Controller
{
	...
	// the original measurements for photo uploads
            ...
            if (!empty($temp_fn)) {
                ...
                if (($temp_fn_ext == 'jpg') || ($temp_fn_ext == 'png') || ($temp_fn_ext == 'jpeg')) {
                    if ($_FILES['uploadFile']['size'] > 5242880) {
                        $this->session->set_flashdata('alert_msg', array('failure', 'Add New Product', 'Upload file size must be less than 5MB!'));
                        redirect(base_url().'products/addproduct');
                ...
                if (!empty($mainPhoto_fn)) {
                    ...
                    if (!$this->upload->do_upload('uploadFile')) {
                       ...
                       // $width_array = array(100, 200);
						...
						//this is the original measurements

						
                    } else {
                        $width_array = array(200, 300);
                        $height_array = array(200, 300);
                        $dir_array = array('xsmall', 'small');
